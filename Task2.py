...
idList = DiskList()
dateList = DiskList()
stationList = DiskList()
tempList = DiskList()
humList = DiskList()

for col in file:
    idList.append(col[fieldnames[0]])
    dateList.append(col[fieldnames[1]])
    stationList.append(col[fieldnames[2]])
    tempList.append(col[fieldnames[3]])
    humList.append(col[fieldnames[4]])
...
positions = []
for i in range(len(dateList)):
    if dateList[i][5:9] in ("2010", "2020") and stationList[i] == 'Changi':
        positions.append(i)
...
for i in range(len(positions)):
    id2.append(idList[positions[i]])

for i in range(len(positions)):
    timestamp2.append(dateList[positions[i]])

for i in range(len(positions)):
    station2.append(stationList[positions[i]])

for i in range(len(positions)):
    temperature2.append(tempList[positions[i]])

for i in range(len(positions)):
    humidity2.append(humList[positions[i]])
